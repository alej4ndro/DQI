...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:a="http://schemas.openehr.org/v1" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" >
	...
	<xsl:template match="/">
		<html>
			...
			<body  id="public" onload="replica();">
					...
					<xsl:if test="a:archetype/a:definition/a:rm_type_name = ('ELEMENT' or 'ENTRY' or 'SECTION' or 'CLUSTER')">
	...
	<xsl:template name="CUERPOCLUSTER">
		...
		<xsl:if test="$i &lt;= $count">
			...
			<div id="{$idcontainer}" idclon="{$idcontainer}" style="margin-left: {$margin}px;margin-top:0px;" class="desmarca" onmouseover="markcontainer('{$jqidcontainer}','{$jqimgclinical}','{$jqimgtechnical}','yes')" onmouseout="unmarkcontainer('{$jqidcontainer}','{$jqimgclinical}','{$jqimgtechnical}','yes')"  type="CLUSTER">
				...
				-->
				<xsl:choose>
					<xsl:when test="a:attributes[a:rm_attribute_name = 'structure_type']/a:children/a:attributes/a:children/a:item/a:list = 'STRC02' and a:attributes[a:rm_attribute_name = 'parts']/a:children/a:attributes[a:rm_attribute_name = 'structure_type']/a:children/a:attributes/a:children/a:item/a:list = 'STRC01'">
						<!-- <xsl:for-each select="a:attributes/a:children/a:attributes[a:rm_attribute_name = 'parts']/a:children">
							<li><xsl:call-template name="titulo"></xsl:call-template></li>
							</xsl:for-each> -->
						<xsl:for-each select="a:attributes[a:rm_attribute_name = 'parts']" >
	...
	<!-- Definicion comportamiento SIMPLE_TEXT -->
	<xsl:template match="*[a:rm_type_name='SIMPLE_TEXT' ]" name="ST">
	...
	<xsl:template match="*[a:rm_type_name='TS']">
	...
	<xsl:template match="*[a:rm_type_name='BL']">
	...
	<!-- Definicion de template CODED_TEXT -->
	<xsl:template match="*[a:rm_type_name='CODED_TEXT']">
	...
	<xsl:template match="*[a:rm_type_name='DATE']">
	...
	<xsl:template match="*[a:rm_type_name='INT']" name="INT">
				...
				<div min="{$min}" max="{$max}" id="{$id}" idclon="{$id}" class="desmarca" style="margin-left:10px" onmouseover="markcontainer('{$jqid}','{$jqimgclinical}','{$jqimgtechnical}','no')" onmouseout="unmarkcontainer('{$jqid}','{$jqimgclinical}','{$jqimgtechnical}','no')" >
					...
					<select name="ord" style="width: 180px">
						<xsl:for-each select="parent::a:attributes/./a:children">
							<xsl:if test="a:rm_type_name = 'ORD'">
	...
	<xsl:template match="*[a:rm_type_name='ORD']">
	...
	<xsl:template match="*[a:rm_type_name='ED']">
		<xsl:param name="alternativa" />
		<xsl:param name="idcontenedor"/>
	...
	<!-- Definicion de template PQ -->
	<xsl:template match="*[a:rm_type_name='PQ']">
		<xsl:param name="alternativa" />
		<xsl:param name="idcontenedor"/>
	...
	<xsl:template name="numnode">
			...
			<xsl:variable name="valorlist" >
				<xsl:choose>
					<xsl:when test="a:attributes/a:children/a:item/a:list">
						<xsl:value-of select="count(a:attributes/a:children/a:item/a:list)"/>
					</xsl:when>
					<xsl:otherwise>0</xsl:otherwise>
				</xsl:choose>
			</xsl:variable>
			<xsl:variable name="valorrange" >
				<xsl:choose>
					<xsl:when test="a:attributes/a:children/a:item/a:range">
						<xsl:value-of select="count(a:attributes/a:children/a:item/a:range)"/>
					</xsl:when>
					<xsl:otherwise>0</xsl:otherwise>
				</xsl:choose>
			</xsl:variable>
			...
			<xsl:if test="$valorlist >= 1 and $valorrange = 0" >
				<xsl:if test="$valorlist > 1">
					<select >
						<xsl:for-each select="a:attributes/a:children/a:item/a:list">
							<option value="{.}" ><xsl:value-of select="."/></option>
						</xsl:for-each>
					</select><span class="{$obligatory}"></span>
				</xsl:if>
			...
			<xsl:if test="$valorlist = 0 and $valorrange >= 1" >
				<xsl:if test="$valorrange >= 1">
					<input type="text" title="Value Min: {a:attributes/a:children/a:item/a:range/a:lower} Value Max: {a:attributes/a:children/a:item/a:range/a:upper} "/>
					<!-- <br/>
